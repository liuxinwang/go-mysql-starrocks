...
func (pos *MysqlPosition) initDb() {
	...
	initPositionData := MysqlBasePosition{BinlogName: "", BinlogPos: 0, BinlogGTID: ""}
...
func (pos *MysqlPosition) SavePosition() error {
	...
	// save pos to db
	marshal, err := json.Marshal(pos.MysqlBasePosition)
	if err != nil {
		log.Fatalf("`position` data parsing failed. err: %v", err.Error())
	}
...
func (pos *MysqlPosition) executeSQL(cmd string, args ...interface{}) (rr *mysql.Result, err error) {
	pos.connLock.Lock()
